...
geometry_msgs::Twist vel_set; //即将发给无人机的速度的期望值
...
int main(int argc, char **argv)
{
    ...
    ros::Publisher vel_sp_pub = nh.advertise<geometry_msgs::Twist>("setpoint_velocity/cmd_vel_unstamped", 10);
    ...
    while(ros::ok())
    {
        ...
        //对即将发给飞控的指令进行赋值
        //pos_setpoint.type_mask = 0b111111111111;
        //pos_setpoint.type_mask = (7 << 9) | (7<< 6) | (7 << 3);  //100 111 111 000  位置 + yaw
        ...
        pos_setpoint.acceleration_or_force.x = 5;
        ...
        //cout << "att_target: [X Y Z] : " << " " << att_target.position.x << " [m] "<< att_target.position.y<<" [m] "<< att_target.position.z<<" [m] "<<endl;
        ...
        vel_set.linear.x=20;
         cout << "Twwwwww : " << vel_set.linear.x <<endl;
        vel_sp_pub.publish(vel_set);
