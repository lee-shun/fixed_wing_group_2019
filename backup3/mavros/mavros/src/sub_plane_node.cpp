...
//*********************************************飞机编号*******************************************
//---------------------------------------------解包后提取出的信息------------------------------
uint8_t commander_state;
double commander_latitude;
double commander_longtitude;
uint8_t commander_heigh;
...
const uint8_t PLANE_NUM = 8 + 1;
int flag_find;                             //帧头寻找 Flag
size_t n_last, n_present;                  //指针*
uint8_t buffer_rx_precent[PLANE_NUM][128]; //Rbuffer_rx 分割帧后   01-0a
int n_rx_precent[PLANE_NUM];               //buffer_rx_precent[x] 中的有效字节数
...
//dji_sdk::Received_target_data target_data;          //发布命令指令(通信模块 -> ROS)   todo
...
//##################################################            回调函数         ############################################################//
void Plane_State_data_callback(const mavros_msgs::sub_plane_msgs::ConstPtr &msg) //待完成
{
    ...
    mavros_msgs::sub_plane_msgs WaitSend_data = *msg;
    uint8_t sub_plane_state = WaitSend_data.sub_plane_state;
    ...
    * */
    ...
    //------测试
    sub_plane_state = 0x01;

    buffer_tx[n_tx_current_flag][0] = CONFIG_ID;
    buffer_tx[n_tx_current_flag][1] = sub_plane_state;
}
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber Plane_State_data_Sub = nh.subscribe<mavros_msgs::sub_plane_msgs>("sub_plane_msgs", 10, Plane_State_data_callback);

    //需要执行的指令  --通过ROS发布给mission_thrower.cpp
    ros::Publisher receive_dataPub = nh.advertise<mavros_msgs::commander_msgs>("commander_msgs", 10);
    ...
    while (ros::ok())
    {
        ...
        if (Rn != 0)
        {
            ...
            if (n_present != 0)
            {
                ...
                outfile << endl
                        << "-- EB 90 -- is not Rbuffer_rx[0][1]" << endl;
                outfile << "Rbuffer_rx : ";
                for (int i = 0; i < Rn; i++)
                {
            ...
            while (n_present < Rn)
            {
                ...
                { //数据复制到对应的ID后
                    ...
                    outfile << endl
                            << "n_rx_precent[" << dec << (int)sendID << "] , n=" << (int)n_rx_precent[sendID] << endl;
                ...
                { // !!!!串口读取有错误
                    outfile.open("/home/ubuntu/serial_error/Rbuffer_rx.txt", ios::app);
                    outfile << endl
                            << "-- n < 8 -- Rbuffer_rx error -- ID: " << dec << (int)sendID << endl;
                    outfile << "Rbuffer_rx : ";
                    for (int i = 0; i < Rn; i++)
                    {
                        outfile << hex << (Rbuffer_rx[i] & 0xff) << " ";
                ...
                { // n==8 !!!!上一回的数据可能有错误
                    if (n_rx_precent[sendID] > 0)
                    {
                        outfile.open("/home/ubuntu/serial_error/Rbuffer_rx.txt", ios::app);
                        outfile << endl
                                << " -- n_rx_precent[sendID] >0 error -- ID: " << dec << (int)sendID << endl;
        ...
        else
        {
            ...
            cout << endl
                 << endl
                 << "-------------this is the new data-----------" << endl
                 << "Serial Received NO Data & No device connected !" << endl;
        }
        ...
        /*
        for (sendID = 1; sendID<PLANE_NUM; sendID++)
        {
            if (n_rx_precent[sendID]>0)
            {   //找出包
                //找到开头帧 316B
                ...
                //----------------------------------------          模块数据 包处理&给出一个完整包  完成         --------------------------------------------------//
                //   所需数据存储变量 n_rx_precent[飞机编号]  buffer_rx_precent[发送消息的飞机编号即1][]
                //>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>          模块数据 完整包信息解读+ROS发布           <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<//
                //cout<<"来了来了"<<"flag_complete = "<<flag_complete<<endl;
                //if (flag_complete ==1)
                if (1)
                {   //如果存在一个完整包 则需要解读它
                    //根据自定义消息格式解析完整包
                    ...
                    //模块信息应答!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                    //cout<<"来了来了"<<endl;
                    uint8_t plane_id = buffer_rx_precent[sendID][n_last+3];
                    cout<<"sendID = " << (sendID & 0xff)<<endl;
                    cout<< "plane_id = "<< (plane_id & 0xff) <<endl;
                    cout<< "buffer"<<endl;
                    for (int i = 0;i < 17;i++)
                    {
                        cout<<"\t"<<(buffer_rx_precent[sendID][i] & 0xff)<<endl;
                    }
                    switch (plane_id)
                    {
                    case 0x01: //0x01 侦察机1
                    ...
                    case CONFIG_ID: //0x02 侦察机2
                    {
                        //----------提取数据
                        cout<<"flag_complete ==1"<<endl;
                        int i = 0;
                        commander_latitude = 0;
                        commander_longtitude = 0;
                        for (i = 0; i < 5; i++)
                        {
                            data_lat[i] = (buffer_rx_precent[sendID][n_last+3+2+i] & 0xff);
                            data_lon[i] = (buffer_rx_precent[sendID][n_last+8+2+i] & 0xff);
                        }
                        // for(i=0;i<5;i++)
                        // {
                        ...
                        for(i=0;i<5;i++)
                        {
                            commander_latitude = commander_latitude + data_lat[i];
                            commander_latitude = commander_latitude * 100;
                            commander_longtitude = commander_longtitude + data_lon[i];
                            commander_longtitude = commander_longtitude * 100;
                        }
                        commander_latitude = commander_latitude / (1e10);
                        commander_longtitude = commander_longtitude / (1e10);

                        commander_state = buffer_rx_precent[sendID][n_last+3+1];
                        commander_heigh = buffer_rx_precent[sendID][n_last+8+2+5];
                        //--------------------提取完成

                        //-----测试
                        //ROS_INFO("Commander: planeID：%d \n\tstate:%d\n\tlatitude:%f\n\tlongtitude:%f\n\theigh:%d",
                          //       CONFIG_ID, commander_state, commander_latitude,commander_longtitude,commander_heigh);
                        cout<<"planeID："<< (CONFIG_ID & 0xff) << endl << "\tstate:"<<(commander_state & 0xff)<<endl << "\tlatitude:"
                                << commander_latitude<<endl << "\tlongtitude:"<<commander_longtitude <<endl << "\theigt:"<<(commander_heigh & 0xff)<<endl;
                        //-------------------发布----
                        //---赋值
                        commander.commander_state = commander_state;
                        commander.commander_latitude = commander_latitude;
                        commander.commander_longtitude = commander_longtitude;
                        commander.commander_heigh = commander_heigh;
                        //---发布
                        receive_dataPub.publish(commander);
       ...
       //----------------------------------------          读取各个子机的状态 buffer_rx_precent[sendID][i]        --------------------------------------------------//
        sendID = 1;
        cout<<"buffer_rx_precent: sendID "<<sendID<<endl<<"\t";
        for(int j = 0;j<16;j++)
        {
            cout << hex <<""<< (buffer_rx_precent[sendID][j] & 0xff) << "\t";

        }
        cout<<endl;


        uint8_t plane_id = buffer_rx_precent[sendID][0 + 3];
        cout << "sendID = " << (sendID & 0xff) << endl;
        cout << "plane_id = " << (plane_id & 0xff) << endl;
        cout << "n_last = " << n_last << endl;
        cout << "buffer" << endl;
        for (int i = 0; i < 17; i++)
        {
            cout << "\t" << (buffer_rx_precent[sendID][i] & 0xff) << endl;
        ...
        case CONFIG_ID: //0x02 侦察机2
        {
            ...
            for (i = 0; i < 5; i++)
            {
                data_lat[i] = (buffer_rx_precent[sendID][0 + 3 + 2 + i] & 0xff);
                data_lon[i] = (buffer_rx_precent[sendID][0 + 8 + 2 + i] & 0xff);
            }
            // for(i=0;i<5;i++)
            // {
            ...
            commander_state = buffer_rx_precent[sendID][0 + 3 + 1];
            commander_heigh = buffer_rx_precent[sendID][0 + 8 + 2 + 5];
            //--------------------提取完成

            //-----测试
            //ROS_INFO("Commander: planeID：%d \n\tstate:%d\n\tlatitude:%f\n\tlongtitude:%f\n\theigh:%d",
            //       CONFIG_ID, commander_state, commander_latitude,commander_longtitude,commander_heigh);
            cout << "planeID：" << (CONFIG_ID & 0xff) << endl
                 << "\tstate:" << (commander_state & 0xff) << endl
                 << "\theigt:" << (commander_heigh & 0xff) << endl;
            printf("\tlatitude:%.9f\n\tlongtitude:%.9f\n", commander_latitude, commander_longtitude);
            //-------------------发布----
            //---赋值
            commander.commander_state = commander_state;
            commander.commander_latitude = commander_latitude;
        ...
        if (1)
        {
            ...
            //memcpy(Wbuffer_tx+3, buffer_tx[n_tx_current_flag], 0x02);
            Wbuffer_tx[3] = CONFIG_ID; //buffer_tx[n_tx_current_flag][0];
            Wbuffer_tx[4] = 0x01; //buffer_tx[n_tx_current_flag][1];
            char check_parity = 0x00;
            for (int i = 0; i < 2; i++)
            {
                check_parity = check_parity + buffer_tx[n_tx_current_flag][i];
            }
            Wbuffer_tx[2 + 3] = check_parity; //校验位
