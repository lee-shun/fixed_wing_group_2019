...
public:
    ...
    static int open_portTHS(int comport);
    static int open_portACM(int comport);
...
int Serial::open_portTHS(int comport)                 //通过参数，打开相应的串口
{
    string serial_port_adr("/dev/ttyTHS"+ to_string(comport));
    cout << serial_port_adr << endl;
    int fd = open( serial_port_adr.c_str() , O_RDWR|O_NOCTTY);
    if (-1 == fd)
    {
...
int Serial::open_portACM(int comport)                 //通过参数，打开相应的串口
{
    string serial_port_adr("/dev/ttyACM"+ to_string(comport));
    cout << serial_port_adr << endl;
    int fd = open( serial_port_adr.c_str() , O_RDWR|O_NOCTTY);
    if (-1 == fd)
    {
