...
void commander_callback(const mavros_msgs::commander_msgs::ConstPtr & msg)
{
    ...
    //    cout <<cot<<" "<<"the transfered received_command.commander_tar_id is "<<endl << (received_command.commander_tar_id& 0xff )<<endl;
    //    cout <<cot<<" "<<"the transfered received_command.commander_state is "<<endl << (received_command.commander_state& 0xff) <<endl;
    ...
    //    cout <<cot<<" "<<"the transfered received_command.commander_heigh is "<<endl <<  received_command.commander_heigh <<endl<<endl<<endl;
    //    cot=cot+1;
}
...
int main(int argc, char** argv)
{
    ...
    ros::Publisher sub_plane_pub=nh.advertise<mavros_msgs::sub_plane_msgs>
            ("sub_plane_msgs", 10);

    ros::Subscriber commander_sub = nh.subscribe<mavros_msgs::commander_msgs>
            ("commander_msgs", 100, commander_callback);




    ros::Rate loop_rate(5);                                   //数据状态更新频率 [20Hz]

    //>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>          串口 打开           <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<//
    //串口serial设置&开启
    serial::Serial sp;                                          //创建一个serial类
    serial::Timeout to = serial::Timeout::simpleTimeout(10);   //创建timeout
    sp.setPort("/dev/ttyUSB1");                                 //设置要打开的串口名称
    ...
    while(ros::ok())
    {
        ...
        { //----------------------------------------          读取各个子机的状态 buffer_rx_precent[sendID][i]        --------------------------------------------------//
            ...
            cout <<"sub_plane_id="<< need_to_send_states.sub_plane_id<<endl;
            cout <<"sub_plane_state="<< need_to_send_states.sub_plane_state<<endl;
...
//        //received_command已经接收到
        cout <<" "<<"the transfered received_command.commander_tar_id is "<<endl <<(received_command.commander_tar_id& 0xff ) << endl;
        cout <<" "<<"the transfered received_command.commander_state is "<<endl << (received_command.commander_state& 0xff) << endl;
        cout <<" "<<"the transfered received_command.commander_latitude is "<<endl << received_command.commander_latitude << endl;
        cout <<" "<<"the transfered received_command.commander_longtitude is "<<endl << received_command.commander_longtitude  << endl;
        cout <<" "<<"the transfered received_command.commander_heigh is "<<endl <<  (received_command.commander_heigh& 0xff) << endl;
